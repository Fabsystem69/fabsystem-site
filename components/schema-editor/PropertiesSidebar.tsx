...
function MobileSelectionBar({ title, subtitle, darkMode, onOpen, onDelete, onClose }: { title: string; subtitle: string; darkMode: boolean; onOpen: () => void; onDelete: () => void; onClose: () => void }) {
  function handleDelete() {
    if (window.confirm(`Supprimer « ${title} » du schéma ?`)) onDelete();
  }

  return <div className={`schema-mobile-selection absolute bottom-[calc(env(safe-area-inset-bottom)+5rem)] left-3 right-3 z-30 flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-xl md:hidden ${darkMode ? "border-neutral-700 bg-neutral-900 text-neutral-100" : "border-slate-200 bg-white text-slate-900"}`}>
    ...
    <button type="button" onClick={handleDelete} className="rounded-xl p-2 text-xl text-red-500 hover:bg-red-50" title={`Supprimer ${title}`} aria-label={`Supprimer ${title}`}>⌫</button>
...
function WirePropertiesSidebar({ edge, darkMode }: { edge: NonNullable<ReturnType<typeof useSchemaStore.getState>["edges"][number]>; darkMode: boolean }) {
  ...
  const openCableSizingPopup = useSchemaStore((s) => s.openCableSizingPopup);
